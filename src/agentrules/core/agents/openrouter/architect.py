"""OpenRouter provider implementation of ``BaseArchitect``."""
...
class OpenRouterArchitect(BaseArchitect):
    """Architect implementation backed by OpenRouter's OpenAI-compatible API."""
    ...
    def format_prompt(self, context: dict[str, Any]) -> str:
        return format_analysis_prompt(
            ...
            agent_name=self.name or "OpenRouter Architect",
        ...
        try:
            ...
            if tools_requested and not self._defaults.tools_allowed:
                logger.info(
                    ...
                    self.name or "OpenRouter Architect",
                    self.model_name,
                )
            ...
            if "User not found" in error_msg and "401" in error_msg:
                error_msg += (
                    "\n[bold yellow]Hint:[/bold yellow] 'User not found' (401) often indicates "
                    "an upstream provider error (e.g. Zhipu AI) due to an invalid key."
                )
