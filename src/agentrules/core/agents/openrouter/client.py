"""OpenRouter client helpers implemented on top of the OpenAI SDK."""
...
def get_client(base_url: str | None = None) -> OpenAI | Any:
    """
    Return a cached OpenAI client configured for the OpenRouter endpoint.

    OpenRouter API is OpenAI-compatible, so we reuse the OpenAI SDK.
    """
    ...
    if resolved_base not in _CLIENTS:
        ...
        if not api_key:
            raise ValueError(
                "OPENROUTER_API_KEY is not set. Please configure it using 'agentrules configure' "
...
def execute_chat_completion(payload: dict[str, Any], base_url: str | None = None) -> Any:
    """Execute a Chat Completions request against the OpenRouter endpoint."""
