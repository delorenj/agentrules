"""ZAI (Zhipu AI) client helpers implemented on top of the OpenAI SDK."""
...
def get_client(base_url: str | None = None) -> OpenAI | Any:
    """
    Return a cached OpenAI client configured for the ZAI (Zhipu) endpoint.

    The Zhipu API is OpenAI-compatible, so we reuse the OpenAI SDK.
    """
    ...
    if resolved_base not in _CLIENTS:
        ...
        if not api_key:
            raise ValueError(
                "ZAI_API_KEY is not set. Please configure it using 'agentrules configure' "
...
def execute_chat_completion(payload: dict[str, Any], base_url: str | None = None) -> Any:
    """Execute a Chat Completions request against the ZAI endpoint."""
